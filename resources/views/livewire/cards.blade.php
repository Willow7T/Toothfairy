    ...
    @if ($cards->isNotEmpty())
        <div class="rounded-lg w-full mx-auto">
            <div class="relative">
                ...
                <p class="md:absolute p-1 text-center w-full rounded-b-lg bottom-0 mt-2 z-10 bg-gray-50 text-gray-600"
    ...
    @foreach ($cards->skip(1) as $index => $card)
        <div class="pt-6 rounded-lg mx-10">
            <div class="md:flex md:justify-between {{ $index % 2 == 0 ? 'md:flex-row-reverse' : '' }}">
                <div
                    class="md:flex-shrink-0 rounded-lg text-center  md:w-5/12 md:h-60"style="background-color: rgba(var(--primary-50), var(--tw-bg-opacity));">
                    
                    <img class="rounded-lg h-full w-full object-fill md:w-80 md:mx-auto" src="{{ $card->image }}"
                        alt="Card image">
                </div>
                <div class="p-8 text-wrap md:w-5/12 rounded-lg md:h-60"
